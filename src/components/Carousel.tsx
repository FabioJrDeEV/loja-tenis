...
export function Carousel() {
  ...
  return (
    <div className="relative w-full overflow-hidden rounded-2xl shadow-lg bg-paper">
      <div 
        className="flex transition-transform duration-500 ease-in-out"
        style={{ transform: `translateX(-${currentIndex * 100}%)` }}
      >
        {images.map((image) => (
          <div key={image.id} className="w-full flex-shrink-0 flex items-center justify-center p-8">
            <img 
              src={image.src} 
              alt={image.alt}
      ...
      <div className="absolute bottom-6 left-1/2 -translate-x-1/2 flex gap-3">
        {images.map((_, index) => (
          <button
            ...
            className={`w-3 h-3 rounded-full transition-all duration-300 ${index === currentIndex ? "bg-primary scale-125" : "bg-ink/30 hover:bg-ink/50"}`}
